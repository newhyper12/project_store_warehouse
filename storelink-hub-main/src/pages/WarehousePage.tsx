  ...
  const fetchRequests = useCallback(async (status?: RequestStatus) => {
    ...
    try {
      ...
      const results = await Promise.all(
        statusesToFetch.map(async (s) => ({
          status: s,
          data: await warehouseApi.getDeliveryRequests(s),
        }))
      );
      ...
      toast.error('Failed to load delivery requests. Please check your connection.');
    } finally {
      setIsLoading(false);
    }
  }, []);
        ...
        <div className="container mx-auto flex items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Inventory Management • Radmin VPN</p>
            </div>
          </div>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-16">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {tab.value === 'pending'
                      ? 'New requests from stores will appear here'
                      : `Requests that are ${tab.label.toLowerCase()} will appear here`}
                  </p>
                </div>
              ) : (
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">

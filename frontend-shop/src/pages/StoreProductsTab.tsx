...
export function StoreProductsTab() {
  const [list, setList] = useState<StoreProduct[] | null>(null);
  ...
  const reload = async () => {
    try {
      const [a, b] = await Promise.all([apiProducts.storeProductsManaged(), apiProducts.storeCategories()]);
      setList(a); setCats(b);
...
function ProductFormModal({ open, onClose, initial, cats, onSaved, onError }: {
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial ? "Редактировать товар" : "Новый товар"}>
